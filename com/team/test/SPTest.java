...
public class SPTest {
    ...
    public static byte[] hashMapToByteArray(HashMap<String, Object> map) throws IOException {
        ...
        //双重压缩
        // byte[] double_compressed_bytes = SPTest.compress(compressed_bytes);
    ...
    public static HashMap<String, Object> setUP(byte[] params_bytes,
        ...
        publicParams = SPTest.byteArrayToHashMap(params_bytes);
        SK = SPTest.byteArrayToHashMap(SK_bytes);
    ...
    public static HashMap<String, Object> setUP(int m, int n) throws IOException {
        ...
        byte[] SK_bytes = SPTest.hashMapToByteArray(SK);
    ...
    public boolean verProofPIU(HashMap<String, Object> PI) {
        ...
        // Element W1Prime = (Element) PP.get("W1Prime");
        // Element W2Prime = (Element) PP.get("W2Prime");
        ...
        byte[] sha1Bytes;
        sha1Bytes = sha160(E1.toString() + E1Prime.toString() + MU.toString());
        ...
        if (!value1.equals(value2))
            return false;
        value1 = E2Prime;
        value2 = _g.powZn(z[2]).mul(h1.powZn(z[3])).mul(E2.powZn(c[1]));
        if (!value1.equals(value2))
            return false;
        // 处理 Theta 和 I 的部分
        value1 = Theta[4].mul(Theta[5]);
        value2 = bp.pairing(g1, I4).powZn(z[5]).mul(bp.pairing(g1, g1).powZn(z[4]))
        ...
        value1 = Theta[6].mul(Theta[7]);
        value2 = bp.pairing(h1, I2).powZn(z[6]).mul(bp.pairing(h1, h1).powZn(z[7]))
                .mul(bp.pairing(W1Prime.invert(), I2).powZn(c[3]));
        if (!value1.equals(value2))
            return false;
        value1 = Theta[8].mul(Theta[14]);
        value2 = bp.pairing(g1, g1).powZn(z[8])
                .mul(bp.pairing(g1, I3).powZn(z[9]))
                .mul(bp.pairing(I3, W1.invert()).mul(bp.pairing(g1, I3).invert().powZn(l)).powZn(c[4]));
        if (!value1.equals(value2))
            return false;

        value1 = Theta[9].mul(Theta[15]);
        value2 = bp.pairing(h1, h1).powZn(z[10])
                .mul(bp.pairing(h1, I4).powZn(z[11]))
                .mul(bp.pairing(I4, W1Prime.invert()).mul(bp.pairing(h1, I4).powZn(k).invert()).powZn(c[5]));
        if (!value1.equals(value2))
            return false;

        value1 = Theta[0].mul(Theta[1]);
        value2 = bp.pairing(_g, F2).powZn(z[12]).mul(bp.pairing(g2, g2).powZn(z[13]))
                .mul(bp.pairing(F1.mul(W2), F2).powZn(c[6]));
        if (!value1.equals(value2))
            return false;

        value1 = Theta[2].mul(Theta[3]);
        value2 = bp.pairing(_g, J2).powZn(z[14])
                .mul(bp.pairing(h2, h2).powZn(z[15]))
                .mul(bp.pairing(J1.mul(W2Prime), J2).powZn(c[7]));
        if (!value1.equals(value2))
            return false;

        value1 = Theta[5].mul(Theta[10]);
        value2 = bp.pairing(g1, g1).powZn(z[16])
                .mul(bp.pairing(_g, I1).powZn(z[17]))
                .mul(bp.pairing(E1.mul(W1), I1).powZn(c[8]));
        if (!value1.equals(value2))
            return false;

        value1 = Theta[7].mul(Theta[11]);
        value2 = bp.pairing(h1, h1).powZn(z[18])
                .mul(bp.pairing(_g, I2).powZn(z[19]))
                .mul(bp.pairing(E2.mul(W1Prime), I2).powZn(c[9]));
        if (!value1.equals(value2))
            return false;

        value1 = Theta[8].mul(Theta[12]);
        value2 = bp.pairing(g1, g1).powZn(z[20])
                .mul(bp.pairing(_g, I3).powZn(z[21]))
                .mul(bp.pairing(E1.mul(g1.powZn(l)).mul(W1), I3).powZn(c[10]));
        if (!value1.equals(value2))
            return false;

        value1 = Theta[9].mul(Theta[13]);
        value2 = bp.pairing(h1, h1).powZn(z[22])
    ...
    public HashMap<String, Object> genProofPI2(HashMap<String, Object> OmegaU) {
        ...
        Element E1Prime = (Element) OmegaU.get("E1Prime");
        Element E2Prime = (Element) OmegaU.get("E2Prime");
        Element E1 = (Element) OmegaU.get("E1");
        Element E2 = (Element) OmegaU.get("E2");
        Element MU = (Element) OmegaU.get("MU");
        Element F1 = (Element) OmegaU.get("F1");
        Element F2 = (Element) OmegaU.get("F2");
        Element J1 = (Element) OmegaU.get("J1");
        Element J2 = (Element) OmegaU.get("J2");
        ...
        for (int mju = 0; mju < lRow; mju++) {
            ...
            for (int nv = 0; nv < kCol; nv++) {
                ...
                sha1Bytes = sha160(valueK.toString() + valueG1.toString() + valueG2.toString() + valueG3.toString()
                        + M2SP.toString());
    ...
    //     return byteOut.toByteArray();
    // }
    // public static byte[] compress(byte[] data, int compressionLevel) throws ZstdException {
    ...
    public static void main(String[] args) throws ClassNotFoundException, IOException {
        SPTest server = new SPTest();
        int n = 10;
        int m = 10;
        int l = 3;
        int k = 3;
        HashMap<String, Object> params = SPTest.setUP(m, n);

        byte[] params_bytes = SPTest.hashMapToByteArray(params); // 假设params是定义好的
        System.out.println("param 压缩后长度: " + params_bytes.length);
        HashMap<String, Object> params1 = SPTest.byteArrayToHashMap(params_bytes);

        User.readParams(params1);
        User u1 = new User();
        HashMap<String, Object> pi1 = server.genProofPI1();
        byte[] PI_bytes;
        PI_bytes = SPTest.hashMapToByteArray(pi1);
        System.out.println("PI1 压缩后长度: " + PI_bytes.length);
        HashMap<String, Object> pi_1 = SPTest.byteArrayToHashMap(PI_bytes);
        ...
        PI_bytes = SPTest.hashMapToByteArray(piu);
        //TODO：因为用 字节流压缩的原因，所以会有些微小变化
        //如果不用双重压缩的话，
        System.out.println("PIU 压缩后长度: " + PI_bytes.length);
        HashMap<String, Object> piu_1 = SPTest.byteArrayToHashMap(PI_bytes);
        ...
        PI_bytes = SPTest.hashMapToByteArray(pi2);
        System.out.println("PI2 压缩前长度: " + PI_bytes.length);
        HashMap<String, Object> pi2_1 = SPTest.byteArrayToHashMap(PI_bytes);
